...
const connection = wslinkConnection.createClient();
let session;
const quality = 0.5;
...
export function saveBinaryData(viewport, fileName) {
  ...
  html2canvas(viewport.element).then((canvas) => {
    canvas.toBlob(
      (blob) => {
...
addButtonToToolbar({
  ...
  onClick: () => {
    ...
    saveBinaryData(viewport, 'screenCapture.jpeg');
  },
});
...
async function run() {
  ...
  remoteEventHandler.setTarget(viewport.element);

  const elementEvents = (e) => {
    remoteEventHandler.processEvent(e);
  };

  connection.connect(config).then(async (validClient) => {
    session = validClient.getConnection().getSession();
    session.subscribe('remotecs3d.events.push.subscription', elementEvents);
  });

  viewport.element.addEventListener(EVENTS.CAMERA_MODIFIED, (_) => {
    html2canvas(viewport.element).then((canvas) => {
      canvas.toBlob(
        (blob) => {
          blob.arrayBuffer().then((arrayBuffer) => {
            if (session) {
              ...
              session.call('remotecs3d.update', [
                blob.size,
                blob.type,
                byteArray,
              ]);
